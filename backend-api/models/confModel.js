...
class Conf {
  ...
  // Check if array has exisiting value
  checkForDuplicate(value, arr){
      ...
      this.conf.find({}, function (err, entries) {
        if (err) {
          ...
          for (let x=0; x<entries.length;x++){
            if (!self.checkForDuplicate(entries[x].speaker, allSpeakers)){
              allSpeakers.push(entries[x].speaker);
            }
          }
          resolve(allSpeakers);
          console.log("function getAllSpeakers() returns: ", allSpeakers);
        }
      });
    });
  }
    ...
    return new Promise((resolve, reject) => {
      this.conf.find({}, function (err, entries) {
        if (err) {
          ...
          let currentTags = []
          for (let x=0; x<entries.length;x++){
            currentTags = entries[x].tags;
            for (let y=0; y<currentTags.length;y++){
            if (!self.checkForDuplicate(currentTags[y], allTags)){
              allTags.push(currentTags[y]);
            }
          }
        }
        resolve(allTags);
        console.log("function getAllTags returns: ", allTags);
        }
      });
    });
  }
  ...
  filterSpeakerSearch(arr, target){
    let results = [];
    if(target === " "){
      return arr;
    } else {
      for(let x=0;x<arr.length;x++){
        if (arr[x].speaker === target){
          results.push(arr[x]);
        }
      }
      return results;
  ...
  filterSessionSearch(arr, target){
    let results = [];
    if(target === " "){
      return arr;
    } else {
      for(let x=0;x<arr.length;x++){
        if (arr[x].session === target){
          results.push(arr[x]);
        }
      }
      return results;
  ...
  filterTimeSearch(arr, target){
    target = target.split(",");
    let results = [];
    if(target === " "){
      return arr;
    } else {
      let fromTime = this.formatTime(target[0])
      let toTime = this.formatTime(target[1])

      let currentTime = 0
      for(let x=0;x<arr.length;x++){
        currentTime = this.formatTime(arr[x].time);
  ...
  filterTagSearch(arr, target){
    ...
    if(target === " "){
      ...
      let tagChecklist = [];
      for(let x=0;x<arr.length;x++){
        currentTags = arr[x].tags;
        tagChecklist = target;
        for (let y=0;y<tagChecklist.length;y++){
          for (let z=0;y<currentTags.length;z++){
            if (tagChecklist[y] === currentTags[z]){
              tagChecklist.splice(y,1);
            }
          }
        }
  ...
  filterRatingSearch(arr, target){
    ...
    if(target === " "){
      ...
      target = target.split(",");
      let minRatingTarget = target[0];
      let maxRatingTarget = target[1];
      let avgRating = 0;
      for(let x=0;x<arr.length;x++){
        avgRating = this.getAvg(arr[x].ratings);
        if (avgRating >= minRatingTarget && avgRating <= maxRatingTarget){
          results.push(arr[x]);
        }
      }
      return results;
    ...
    return new Promise((resolve, reject) => {
      this.conf.find({}, function (err, entries) {
        if (err) {
          ...
          searchResults = self.filterSpeakerSearch(entries, speakerName);
          searchResults = self.filterSessionSearch(searchResults, sessionID);
          searchResults = self.filterTimeSearch(searchResults, times);
         // searchResults = self.filterTagSearch(searchResults, tags);
          searchResults = self.filterRatingSearch(searchResults, rating);
          resolve(searchResults);
          console.log("function getSearchResults() returns: ", searchResults);
        }
      });
    });
  }
